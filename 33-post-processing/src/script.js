...
const renderer = new THREE.WebGLRenderer({
  canvas: canvas,
  antialias: true, // Don't work on effect composer
});
...
renderer.outputColorSpace = THREE.SRGBColorSpace; // Don't work on effect composer
renderer.setSize(sizes.width, sizes.height);
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
...
const renderTarget = new THREE.WebGLRenderTarget(800, 600, {
  // 0 is nothing, 2 adding antialias again <3
  // The bigger the number, less performant it is, so be careful
...
tintPass.material.uniforms.uTint.value = new THREE.Vector3(0.2, 0.18, 0.5);
